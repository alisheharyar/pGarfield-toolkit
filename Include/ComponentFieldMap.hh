...
class ComponentFieldMap : public ComponentBase {
 ...
 public:
  ...
  // Enable/disable the caching of the bounding boxes
  void EnableCachingOfBoundingBoxes() { cacheElemBoundingBoxes = true; }
  void DisableCachingOfBoundingBoxes() { cacheElemBoundingBoxes = false; }
  ...
  // Enable/disable the searching through neighbors
  void EnableSearchThroughNeighbors() { searchThroughNeighbors = true; }
  void DisableSearchThroughNeighbors() { searchThroughNeighbors = false; }

  friend class ViewFEMesh;

  int _totalCalls;
  int _totalLastFound;
  int _totalFoundInNeighbors;
 ...
 protected:
  ...
  // Caching the bounding boxes
  // Flag to check if bounding boxes of elements are cached 
  bool cacheElemBoundingBoxes;
  bool elemBoundingBoxesCached; // if the caching is done or not
  ...
  // Flag for searching through neighbors
  bool searchThroughNeighbors;
  bool neighborsComputed;
  int* elemNeighbors;
  ...
  // Initialize neighbors of the tetrahedrons after mesh nodes and elements are initialized
  bool ComputeNeighbors(void);

  void GetElementNeighbours(const int elemIndex, int* neigh);
};
}
