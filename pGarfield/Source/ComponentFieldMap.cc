...
ComponentFieldMap::ComponentFieldMap()
    : is3d(true),
      ...
      elemBoundingBoxesCached(false),
      tetTree(NULL),
      useTetrahedralTreeForSearch(false),
      isTreeInitialized(false),
      searchThroughNeighbors(false),
  ...
  // Temporary variables to measure the hit/miss ratio
  _totalCalls=0;
  _totalLastFound=0;
  _totalFoundInNeighbors=0;
}
...
int ComponentFieldMap::FindElement5(const double x, const double y,
  ...
  if (lastElement > -1 && !checkMultipleElement) {
    ...
    // not in the previous element, so check its neighbours (if enabled)
    if(searchThroughNeighbors) {
      if(!neighborsComputed) {
        if(!ComputeNeighbors()) {
          std::cerr << m_className << "::FindElement5:\n";
          std::cerr << "    Error while computing the element neighbors.\n";
          return -1;
        }
      }

      int neighbours[4];
      GetElementNeighbours(lastElement, neighbours);

      for(int ni=0; ni<4; ni++) {
        int neigh = neighbours[ni];
        if(neigh > -1) {

          xmin = elements[neigh].xmin;
          xmax = elements[neigh].xmax;
          ymin = elements[neigh].ymin;
          ymax = elements[neigh].ymax;
          zmin = elements[neigh].zmin;
          zmax = elements[neigh].zmax;
          ...
          if (elements[neigh].degenerate) {
            rc = Coordinates3(x, y, z, t1, t2, t3, t4, jac, det, neigh);
            if (rc == 0 && t1 >= 0 && t1 <= +1 && t2 >= 0 && t2 <= +1 && t3 >= 0 &&
              t3 <= +1)
              lastElement = neigh;
              return lastElement;
          } else {
            rc = Coordinates5(x, y, z, t1, t2, t3, t4, jac, det, neigh);
            if (rc == 0 && t1 >= -1 && t1 <= +1 && t2 >= -1 && t2 <= +1)
              lastElement = neigh;
              return lastElement;
          }
  ...
  for (int i = 0; i < numElemToSearch; ++i) {
    ...
    if(cacheElemBoundingBoxes) {
      ...
      xmin = std::min(std::min(nodes[elements[idxToElemList].emap[0]].x, nodes[elements[idxToElemList].emap[1]].x),
                    std::min(nodes[elements[idxToElemList].emap[2]].x, nodes[elements[idxToElemList].emap[3]].x));
      xmax = std::max(std::max(nodes[elements[idxToElemList].emap[0]].x, nodes[elements[idxToElemList].emap[1]].x),
                      std::max(nodes[elements[idxToElemList].emap[2]].x, nodes[elements[idxToElemList].emap[3]].x));
      ymin = std::min(std::min(nodes[elements[idxToElemList].emap[0]].y, nodes[elements[idxToElemList].emap[1]].y),
                      std::min(nodes[elements[idxToElemList].emap[2]].y, nodes[elements[idxToElemList].emap[3]].y));
      ymax = std::max(std::max(nodes[elements[idxToElemList].emap[0]].y, nodes[elements[idxToElemList].emap[1]].y),
                      std::max(nodes[elements[idxToElemList].emap[2]].y, nodes[elements[idxToElemList].emap[3]].y));
      zmin = std::min(std::min(nodes[elements[idxToElemList].emap[0]].z, nodes[elements[idxToElemList].emap[1]].z),
                      std::min(nodes[elements[idxToElemList].emap[2]].z, nodes[elements[idxToElemList].emap[3]].z));
      zmax = std::max(std::max(nodes[elements[idxToElemList].emap[0]].z, nodes[elements[idxToElemList].emap[1]].z),
                      std::max(nodes[elements[idxToElemList].emap[2]].z, nodes[elements[idxToElemList].emap[3]].z));
    }
    ...
    if(searchThroughNeighbors) {
      ...
      for(int ni=0; ni<4; ni++) {
        ...
        if(neigh > -1) {
          ...
          rc = Coordinates13(x, y, z, t1, t2, t3, t4, jac, det, neigh);
          if (rc == 0 &&
            t1 >= 0 && t1 <= +1 &&
            t2 >= 0 && t2 <= +1 &&
            t3 >= 0 && t3 <= +1 &&
  ...
  for (int i = 0; i < numElemToSearch; i++) {
    ...
    if (rc == 0 && t1 >= 0 && t1 <= +1 && t2 >= 0 && t2 <= +1 && t3 >= 0 &&
      ...
      if (debug) {
        ...
        std::cout << "    Found matching element " << idxToElemList << ".\n";
      }
      if (!checkMultipleElement) return idxToElemList;
      for (int j = 0; j < 4; ++j) {
        for (int k = 0; k < 4; ++k) jacbak[j][k] = jac[j][k];
...
bool ComponentFieldMap::ComputeNeighbors(void) {
  // Do not proceed if not properly initialised.
  if (!ready) {
    std::cerr << m_className << "::ComputeNeighbors:\n";
    std::cerr << "    Field map not yet initialised.\n";
    std::cerr << "    Element neighbors cannot be computed.\n";
    return false;
  }

  std::cerr << m_className << "::ComputeNeighbors:\n";
  std::cerr << "    Computing the element neighbors.\n";

  // calculate the neighbours
  int* tetraNode = new int[10*elements.size()];
  for(unsigned int e=0; e<elements.size(); e++) {
	  for(int n=0; n<10; n++) {
		  tetraNode[e*10+n] = elements[e].emap[n];
	  }
  }
  
  elemNeighbors = tet_mesh_neighbor_tets (10, elements.size(), tetraNode);
...
void ComponentFieldMap::GetElementNeighbours(const int elemIndex, int* neigh) {
	for(int i=0; i<4; i++)
		neigh[i] = elemNeighbors[elemIndex*4+i];
}
