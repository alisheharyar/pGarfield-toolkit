/*
THIS FILE IS THE TRACK PARALLEL TEST FILE
The file will send each process its own track, where it will calculate
the number of primary electrons, total avalanched electrons, and gain.
The input parameter file is given by card.ini file that should be located in the same directory
*/
...
static void master(int argc, char** argv, int size, int nTracks);
...
main(int argc, char **argv)
{
  ...
  nTracks = param_num_events;
  if(myrank==0) 
    cout << "NUMBER OF PROCESSES: " << size << endl;
    cout << "NUMBER OF EVENTS: " << nTracks << endl;
...
void master(int argc, char** argv, int size, int nTracks) {
...
//  plottingEngine.SetDefaultStyle();
  ...
  const char* title = param_track_particle.c_str();
  TFile *f = new TFile("ntuples.root", "RECREATE");
  TNtuple* ntuple  = new TNtuple("ntuple", title,"xe1:ye1:ze1:te1:e1:xe2:ye2:ze2:te2:e2");
  TNtuple* ntuple1 = new TNtuple("ntuple1", title, "gain:np:ne:ni:nd");

  int iTrack=0, iCompleted=0;
  for(; iTrack<size-2; iTrack++) {
    int slave_id = iTrack + 1;
    //cout << "######## SEND : " << iTrack << endl;
    
    param.trackID = iTrack;
    MPI_Send(&param, 1, mpi_event_param_type, slave_id, WORKTAG, MPI_COMM_WORLD);
  }

  while(iCompleted<nTracks) {
    
    MPI_Recv(&res, 1, mpi_event_result_type, 
      MPI_ANY_SOURCE, RESULT_TAG, MPI_COMM_WORLD, &status);
    
    //cout << "######## RECV : " << res.trackID << endl;
    ...
    out << "Event ID: " << res.trackID << ", ne= " <<  res.ne << endl;

    // Accumulate the results
    //hElectrons->Fill(res.ne);
    //hIons->Fill(res.ni);
    double gain = (double)res.ne / (double)res.np;
    ntuple1->Fill(gain,res.np,res.ne,res.ni,res.nd);
    ...
    if(iTrack<nTracks) {
      ...
      //MPI_Send(&iTrack, 1, MPI_INT, status.MPI_SOURCE, WORKTAG, MPI_COMM_WORLD);    
      iTrack++;
    }
  }
...
*/
  ...
  // Create the track
  double momentum = param_track_momentum;

  track = new TrackHeed();
  track->SetSensor(sensor);
  track->SetParticle(param_track_particle);
  track->DisableDeltaElectronTransport(); //ONLY DELTA ELECTRONS PRODUCED
...
void do_work(const EventParam& param, EventResult& res,
    ...
    res = { 0 };
    res.trackID = param.trackID;

    // The initial impact position of the incoming ionising track
    // Weston Cadena
    const double pitch = param_dim_pitch;
    const double smear = pitch / 2.;
    double track_x = -smear + RndmUniform() * smear;
    double track_y = -smear + RndmUniform() * smear;
    double track_z = param_sensor_z2 - .001;
    double t0 = 0.0;
    double e0 = 0.1;
    double tMin = 0.0;
    int Total_Number_Electrons = 0;
    int Total_Electrons_Along_Track = 0;
    int Total_Energy_Loss = 0.;
    int Total_Number_Clusters;
    // Momentum direction of incoming track
    double track_dx = 0.0;
    double track_dy = 0.0; //
    double track_dz = -1.0; //Track is downstream

    //GetCluster variables
    double xcls, ycls, zcls, tcls, e, extra;
    int nclus = 0;
    //GetElectron variables
    double xele, yele, zele, tele, eele, dxele, dyele, dzele;

    //GENERAL TRACK ANSWERS 
    int npe_t = 0; // num primary electrons
    int ne_t = 0; // num total electrons avalanched
    int ni_t = 0; // num total ions
    int distribution_t = 0; // num of electrons avalanched that reached the bottom of the sensor
    int positions = 0;
    ...
    //std::cout << myrank << ": NEW TRACK CALLED" << std::endl;
    while (track->GetCluster(xcls, ycls, zcls, tcls, nclus, e, extra))
    {
        // Number of primary electrons in collision
        npe_t += nclus;

        // Avalanching Each Primary Electron
        for (int iclus = 0; iclus < nclus; iclus++)
        {
          
          track->GetElectron(iclus, xele, yele, zele, tele, eele, dxele, dyele, dzele);
          ...
          ne_t += ne;
          ni_t += ni;
          //std::cout << std::fixed << std::setprecision(7) << myrank << ": Elecctron: X0=" << xele << " Y0=" << yele << " Z0=" << zele << " t0=" << tele << " e0=" << eele;
          //std::cout << std::fixed << std::setprecision(7) << " DX0=" << dxele << " DY0=" << dyele << " DZ0=" << dzele << std::endl;
          //std::cout << myrank << ": Track " << param.trackID << ": Avalanche Size: ne=" << ne << ", ni=" << ni << std::endl;

            
          // Finding start points and endports of each electron and ion
          ...
          for (int j = np; j--;) {
              ...
              // if it reached the sensor
              if(ze2 <= param_sensor_z1 + .001){
                distribution_t++;
              }
            } // Endpoints loop
    ...
    } // Get Cluster While loop
        res.np = npe_t;
        res.ne = ne_t;
        res.ni = ni_t;
        res.nd = distribution_t;
        res.tupleSize = v_xe1.size();
        std::cout << myrank << ": Number of enpoints= " << positions << " Number of Pimary= " << npe_t << std::endl; 
        std::cout << myrank << ": ##TOTAL##  TRACK " << param.trackID << ": Avalanche Size: ne_t=" << ne_t << ", ni_t=" << ni_t << std::endl;
...
void create_mpi_event_param_type(void) {
  ...
  MPI_Aint displ[1];
  MPI_Get_address(&param.trackID, &displ[0]);
...
void create_mpi_event_result_type(void) {
  ...
  MPI_Datatype old_types[6] = {MPI_INT,MPI_INT,MPI_INT,MPI_INT,MPI_INT,MPI_INT};

  MPI_Aint displ[6];
  MPI_Get_address(&res.trackID, &displ[0]);
  MPI_Get_address(&res.np, &displ[1]);
  MPI_Get_address(&res.ne, &displ[2]);
  MPI_Get_address(&res.ni, &displ[3]);
  MPI_Get_address(&res.nd, &displ[4]);
  MPI_Get_address(&res.tupleSize, &displ[5]);
